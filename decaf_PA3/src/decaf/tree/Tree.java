...
public abstract class Tree {
	 ...
	 * Array repeat.
	 */
	public static final int ARRAYREPEAT = VARSTMT + 1;

	/**
	 * Default access
	 */
	public static final int DEFAULTACCESS = ARRAYREPEAT + 1;

	/**
	 * For each
	 */
	public static final int FOREACH = DEFAULTACCESS + 1;
    ...
    public abstract static class LValue extends Expr {
    	...
    	public enum Kind {
    		LOCAL_VAR, PARAM_VAR, MEMBER_VAR, ARRAY_ELEMENT, VAR_VAR
    	}
    	public Kind lvKind;
	...
	public static class Foreach extends Tree {
		...
		public Foreach(TypeLiteral type, String name, Expr range, Expr condition, Tree stmt, 
				Location loc) {
			super(FOREACH, loc);
			this.type= type;
			this.name = name;
			...
			if (this.block.block == null) {
				this.block = new Block(new ArrayList<Tree>(), stmt.loc);
				this.block.block.add(stmt);
			}
		}
