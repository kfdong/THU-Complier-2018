...
public class TypeCheck extends Tree.Visitor {
	...
	public void visitAssign(Tree.Assign assign) {
		...
		if (assign.left instanceof Tree.Var) {
			assign.left.type = assign.expr.type;
			// update symbol
			table.updateType(((Tree.Var)(assign.left)).name, assign.expr.type);
			assign.left.lvKind = Tree.LValue.Kind.VAR_VAR;
		} else {
	...
	public void visitScopy(Tree.Scopy scopy) {
		...
		if (!scopy.ident.type.isClassType()) {
			...
			if (!scopy.expr.type.isClassType()) {
				if (!scopy.expr.type.equal(BaseType.ERROR))
					issueError(new BadScopyArgError(scopy.expr.getLocation(), "src", scopy.expr.type.toString()));
			}
		} else {
			// modified based on specific in PA3
			// if (!scopy.ident.type.compatible(scopy.expr.type)) 
			if (!scopy.ident.type.equal(scopy.expr.type)) 
				issueError(new BadScopySrcError(scopy.getLocation(), scopy.ident.type.toString(), 
	...
	public void visitGuarded(Tree.Guarded guarded) {
		...
		for (int i = 0; i < n; ++i) {
			...
			// System.out.printf("%s\n", guarded.slist.get(i).getClass().getName());
			guarded.slist.get(i).accept(this);
			if (!guarded.elist.get(i).type.equal(BaseType.BOOL)) {
				issueError(new BadTestExpr(guarded.elist.get(i).getLocation()));
			}
		}
	}
	...
	public void visitArrayRepeat(Tree.ArrayRepeat arrayRepeat) {
		arrayRepeat.array.accept(this);
		arrayRepeat.type = new ArrayType(arrayRepeat.array.type);
		if (arrayRepeat.array.type.equal(BaseType.ERROR) ||
				arrayRepeat.array.type.equal(BaseType.UNKNOWN)) {
			issueError(new BadArrElementError(arrayRepeat.array.getLocation()));
			arrayRepeat.type = BaseType.ERROR;
		}
		arrayRepeat.number.accept(this);
		if (!arrayRepeat.number.type.equal(BaseType.INT)) {
			issueError(new BadArrTimesError(arrayRepeat.number.getLocation()));
		}
	}

	@Override
	public void visitDefaultAccess(Tree.DefaultAccess defaultAccess) {
		defaultAccess.array.accept(this);
		defaultAccess.index.accept(this);
		defaultAccess.deflt.accept(this);
		if (!defaultAccess.array.type.isArrayType()) {
			issueError(new BadArrOperArgError(defaultAccess.array.getLocation()));
			if (!defaultAccess.deflt.type.equal(BaseType.UNKNOWN) && 
					!defaultAccess.deflt.type.equal(BaseType.ERROR))
				defaultAccess.type = defaultAccess.deflt.type;
			else {
				defaultAccess.type = BaseType.ERROR;
				// TODO
			}
		} else {
			if (!((ArrayType) defaultAccess.array.type).getElementType().compatible(
						defaultAccess.deflt.type))
				issueError(new BadDefError(defaultAccess.index.getLocation(),
							((ArrayType) defaultAccess.array.type).getElementType().toString(),
							defaultAccess.deflt.type.toString()));
			defaultAccess.type = ((ArrayType) defaultAccess.array.type)
					.getElementType();
		}
		if (!defaultAccess.index.type.equal(BaseType.INT)) {
			issueError(new BadArrIndexError(defaultAccess.index.getLocation()));
		}
	}
	...
	public void visitForeach(Tree.Foreach foreach) {
		...
		if (foreach.type.type.equal(BaseType.UNKNOWN)) {
			if (!foreach.range.type.isArrayType()) {
				...
				foreach.type.type = BaseType.ERROR;
			} else {
				foreach.type.type = ((ArrayType)(foreach.range.type)).getElementType();
			}
			table.updateType(foreach.name, foreach.type.type);
		} else {
			if (!foreach.range.type.isArrayType()) {
				issueError(new BadArrOperArgError(foreach.range.getLocation()));
			} else {
				if (foreach.type.type.equal(BaseType.VOID)) {
					issueError(new BadVarTypeError(foreach.getLocation(), foreach.name));
				} else {
					Type expected = foreach.type.type;
					Type given = ((ArrayType)(foreach.range.type)).getElementType();
					if (!given.compatible(expected)) 
		...
		if (foreach.condition != null) {
			...
			if (!foreach.condition.type.equal(BaseType.BOOL)) {
				issueError(new BadTestExpr(foreach.condition.getLocation()));
			}
		}
		breaks.add(foreach);
