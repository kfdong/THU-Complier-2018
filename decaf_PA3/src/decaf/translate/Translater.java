...
public class Translater {
	...
	public void genCheckNewArraySize(Temp size, boolean isRepeat) {
		Label exit = Label.createLabel();
		Temp cond = genLes(size, genLoadImm4(0));
		genBeqz(cond, exit);
		Temp msg;
		if (isRepeat) msg = genLoadStrConst(RuntimeError.NEGATIVE_REPEAT_SIZE);
	...
	public void genCheckDiv(Temp denominator) {
		Label exit = Label.createLabel();
		genBeqz(genEqu(denominator, genLoadImm4(0)), exit);
	...
	public Temp genNewArray(Temp length, boolean isRepeat) {
		genCheckNewArraySize(length, isRepeat);
