...
public class Parser extends Table {
     ...
     */
    private SemValue parse(int symbol, Set<Integer> F) {
		Set<Integer> begin = beginSet(symbol);
		Set<Integer> follow = followSet(symbol);
		Set<Integer> end = followSet(symbol); end.addAll(F);

		if (!begin.contains(lookahead) && !end.contains(lookahead)) {
			error();
			while (!begin.contains(lookahead) && !end.contains(lookahead)) 
				lookahead = lex();
		}
