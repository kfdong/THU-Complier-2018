	...
	public void analyzeDUChain() {
        ...
        do {
            ...
            for (BasicBlock bb : bbs) {
                for (int i = 0; i < 2; i++) {
                    if (bb.next[i] >= 0) { // Not RETURN
                        bb.extMerge(bb.extLiveOut, bbs.get(bb.next[i]).extLiveIn);
                    }
                }
                bb.extRemove(bb.extLiveOut, bb.extDef);
                if (bb.extMerge(bb.extLiveIn, bb.extLiveOut))
                    changed = true;
                for (int i = 0; i < 2; i++) {
                    if (bb.next[i] >= 0) { // Not RETURN
                        bb.extMerge(bb.extLiveOut, bbs.get(bb.next[i]).extLiveIn);
		...
		/*
		for (BasicBlock bb : bbs) {
			System.out.printf("%d ", bb.bbNum);
			System.out.println(bb.extLiveOut);
		}*/
	}
